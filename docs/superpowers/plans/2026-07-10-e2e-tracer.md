...
**Goal:** 핵심 문항 4개(지역/전용면적/연소득/순자산) → 클라이언트 JSON 규칙 데이터로 "버팀목전세자금(일반)" 1개 상품 판정 → 결과 화면(충족/미충족 배지+사유)까지 관통하는 최소 동작 파이프라인을 만든다.

**Architecture:** Next.js(App Router) + TypeScript + Tailwind. 자격 판정은 순수 함수(`lib/eligibility.ts`)가 JSON 규칙 데이터(`data/products/bootmok-general.json`)를 읽어 계산하며 서버 요청 없이 브라우저에서만 실행된다. 문답은 `app/page.tsx`의 클라이언트 컴포넌트가 로컬 state(`useState`)로 4단계를 관리하고, 마지막에 판정 함수를 호출해 결과를 같은 페이지에 렌더링한다.
...
- Produces:
  - `type Applicant = { region: 'capital' | 'non-capital'; areaSqm: number; annualIncomeKrw: number; netAssetKrw: number }`
...
describe('checkBootmokGeneral', () => {
  ...
  it('수도권 보증금이 3억원을 초과하면 보증금 초과 사유와 함께 eligible: false를 반환한다', () => {
    const result = checkBootmokGeneral({ ...baseApplicant, region: 'capital' })
    // depositKrw는 Applicant에 없으므로 이 테스트는 area/보증금 대신 지역별 면적 초과 케이스로 대체
...
export function checkBootmokGeneral(applicant: Applicant): EligibilityResult {
  ...
  void depositLimit // 보증금 입력값은 Task 4에서 Applicant에 추가 예정, 현재 트레이서 범위 밖
...
const STEPS = ['region', 'area', 'income', 'asset'] as const
...
git commit -m "feat: 4단계 문답 위저드 컴포넌트(지역/면적/소득/순자산)"
```
...
Expected: 지역 선택 → 면적 → 소득 → 순자산 입력 후 "결과 보기" 클릭 시 결과 카드가 렌더링됨.
브라우저 개발자도구 Network 탭에서 XHR/fetch 요청이 발생하지 않음을 확인(서버 전송 없음 검증).

- [ ] **Step 6: 테스트 스위트 최종 확인**

Run: `npm test`
Expected: PASS — Task 2에서 작성한 5개 테스트 전부 통과 유지
